...
def helper():
        ...
        print("------------")
        print("\t./rapidscan.py example.com: verifica o domínio example.com.")
        ...
        print("--------")
        print("\t["+proc_high+"]: O processo de digitalização pode levar mais tempo (não previsível).")
        print("\t["+proc_med+"]: O processo de digitalização pode levar menos de 10 minutos.")
        print("\t["+proc_low+"]: O processo de digitalização pode levar menos de um minuto ou dois.")
        print(bcolors.OKBLUE+"Vulnerability Information:"+bcolors.ENDC)
        print("--------------------------")
        print("\t"+vul_info('c')+": Requer atenção imediata, pois pode levar a comprometimento ou indisponibilidade do serviço.")
        print("\t"+vul_info('h')+"    : Pode não levar a um compromisso imediato, mas há chances consideráveis ​​de probabilidade.")
        print("\t"+vul_info('m')+"  : O invasor pode correlacionar várias vulnerabilidades desse tipo para lançar um ataque sofisticado.")
        print("\t"+vul_info('l')+"     : Não é um problema sério, mas é recomendável cuidar da descoberta.")
        print("\t"+vul_info('i')+"    : Não classificado como vulnerabilidade, apenas um alerta informativo útil a ser considerado.\n")
...
def logo():
    ...
    logo_ascii = """
                                ...
                                / ( (//)/(/__)( (//)
                                     /
                     """+bcolors.ENDC+"""(O verificador de vulnerabilidades da Web de várias ferramentas)
...
if args_namespace.help or (not args_namespace.update \
    ...
    if len(rs_vul_list)==0:
        print("\t"+bcolors.OKGREEN+"Nenhuma vulnerabilidade detectada."+bcolors.ENDC)
    else:
    ...
    print("\tTotal Número de verificações de vulnerabilidade         : "+bcolors.BOLD+bcolors.OKGREEN+str(len(tool_names))+bcolors.ENDC)
    print("\tTotal Número de verificações de vulnerabilidade ignoradas: "+bcolors.BOLD+bcolors.WARNING+str(rs_skipped_checks)+bcolors.ENDC)
    print("\tTotal Número de vulnerabilidades detectadas        : "+bcolors.BOLD+bcolors.BADFAIL+str(len(rs_vul_list))+bcolors.ENDC)
    print("\tTotal Tempo decorrido para a verificação           : "+bcolors.BOLD+bcolors.OKBLUE+display_time(int(rs_total_elapsed))+bcolors.ENDC)
    print("\n")
    print("\tFor Para fins de depuração, você pode visualizar a saída completa gerada por todas as ferramentas denominadas "+bcolors.OKBLUE+debuglog+bcolors.ENDC+" no mesmo diretório.")
