...
# Iniciar gancho para permitir que plugins carreguem bancos de dados etc.
run_hooks("", "start");

# Agora que fizemos o precursor, faça a varredura
foreach my $mark (@MARKS) {
...
#################################################################################
# Carregar arquivos de configuração em ordem
sub config_init {
    my (@CF, $home);
    my $config_exists = 0;
...
######################################################################
## check_config_defined(item, padrão)
## Verifica se a configuração foi definida, avisa e define para um padrão
